...
class PressurePlate(gym.Env):
    ...
    def __init__(self, height, width, n_agents, sensor_range, layout):
        ...
        self.obs_space_dim = (sensor_range + 1) * (sensor_range + 1) * 5 + 2

        self.observation_space = spaces.Tuple(tuple(
            n_agents * [spaces.Box(np.array([0] * self.obs_space_dim), np.array([1] * self.obs_space_dim))]
        ))
    ...
    def step(self, actions):
        ...
        for i,plate in enumerate(self.plates):
            if not plate.pressed:
                for x,y in zip(self.doors[plate.id].x,self.doors[plate.id].y):
